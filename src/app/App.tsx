...
import { ServiceCatalogPage } from '../pages/provider/ServiceCatalogPage';
...
import { getRouteFromUrl, buildRouteUrl } from './routes';
...
export function App() {
  ...
  const setCurrentRoute = (route: PageRoute) => {
    ...
    if (route === 'provider-onboarding') {
      normalizedRoute = 'provider-onboarding/user-details';
    }
    if (normalizedRoute !== currentRoute) {
      const url = buildRouteUrl(normalizedRoute);
      window.history.pushState({ route: normalizedRoute }, '', url);
      setCurrentRouteState(normalizedRoute);
    }
  };
  ...
  useEffect(() => {
    ...
    const normalizedInitial =
      initialRoute === 'provider-onboarding'
        ? 'provider-onboarding/user-details'
        : initialRoute;

    window.history.replaceState(
      { route: normalizedInitial },
      '',
      buildRouteUrl(normalizedInitial)
    );
    if (normalizedInitial !== initialRoute) {
      setCurrentRouteState(normalizedInitial);
    }
  ...
  const handleToggleRole = async () => {
    ...
    if (user.role === 'consumer') {
      setCurrentRoute('provider-onboarding/user-details');
      return;
    }
  ...
  const handleApplicationApproved = (updatedUser: User, initialService?: Service) => {
    ...
    const displayName =
      updatedUser.providerDetails?.businessName ||
  ...
  // Render current page
  const renderPage = () => {
    if (currentRoute.startsWith('provider-onboarding')) {
      return (
